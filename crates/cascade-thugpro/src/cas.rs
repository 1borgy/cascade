...
const CUSTOM_SKATER_NAME: &'static str = "data.CustomSkater";
const CUSTOM_SKATER_ID: qb::Id = qb::Id::Checksum(314551426);

const STORY_SKATER_NAME: &'static str = "data.StorySkater";
...
const CUSTOM_NAME: &'static str = "data.CustomSkater.custom";
const CUSTOM_ID: qb::Id = qb::Id::Compress8(195);
...
impl CustomSkater {
    pub fn modify(&self, custom_skater: &mut Box<qb::Structure>) -> Result<()> {
        if let Some(custom) = &self.custom {
            custom.modify(expect_structure_mut(custom_skater, CUSTOM_ID, CUSTOM_NAME)?)?;
        }
        Ok(())
    }
}

const APPEARANCE_NAME: &'static str = "data.CustomSkater.custom.appearance";
const APPEARANCE_ID: qb::Id = qb::Id::Checksum(1431076207);

const INFO_NAME: &'static str = "data.CustomSkater.custom.info";
...
impl TryFrom<Box<qb::Structure>> for Custom {
    ...
    fn try_from(custom: Box<qb::Structure>) -> Result<Self> {
        let appearance = expect_structure(Box::clone(&custom), APPEARANCE_ID, APPEARANCE_NAME)?;
...
impl Custom {
    pub fn modify(&self, custom: &mut Box<qb::Structure>) -> Result<()> {
        if let Some(scales) = &self.scales {
            let appearance = expect_structure_mut(custom, APPEARANCE_ID, APPEARANCE_NAME)?;
...
const TRICK_MAPPING_NAME: &'static str = "data.CustomSkater.custom.info.trick_mapping";
const TRICK_MAPPING_ID: qb::Id = qb::Id::Compress8(61);

const SPECIALS_NAME: &'static str = "data.CustomSkater.custom.info.specials";
const SPECIALS_ID: qb::Id = qb::Id::Compress8(64);
...
const BOARD_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(208);
const FEET_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(207);
const HANDS_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(204);
const HEAD_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(199);
const HEADTOP_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(196);
const JAW_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(197);
const LOWER_ARM_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(203);
const LOWER_LEG_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(206);
const NOSE_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(198);
const OBJECT_SCALING_ID: qb::Id = qb::Id::Compress8(209);
const STOMACH_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(201);
const TORSO_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(200);
const UPPER_ARM_BONE_GROUP_ID: qb::Id = qb::Id::Compress8(202);
...
impl TryFrom<Box<qb::Structure>> for Scales {
    ...
    fn try_from(structure: Box<qb::Structure>) -> Result<Self> {
        Ok(Self {
            board_bone_group: Scalable::from(structure.get(BOARD_BONE_GROUP_ID).cloned()),
            feet_bone_group: Scalable::from(structure.get(FEET_BONE_GROUP_ID).cloned()),
            hands_bone_group: Scalable::from(structure.get(HANDS_BONE_GROUP_ID).cloned()),
            head_bone_group: Scalable::from(structure.get(HEAD_BONE_GROUP_ID).cloned()),
            headtop_bone_group: Scalable::from(structure.get(HEADTOP_BONE_GROUP_ID).cloned()),
            jaw_bone_group: Scalable::from(structure.get(JAW_BONE_GROUP_ID).cloned()),
            lower_arm_bone_group: Scalable::from(structure.get(LOWER_ARM_BONE_GROUP_ID).cloned()),
            lower_leg_bone_group: Scalable::from(structure.get(LOWER_LEG_BONE_GROUP_ID).cloned()),
            nose_bone_group: Scalable::from(structure.get(NOSE_BONE_GROUP_ID).cloned()),
            object_scaling: Scalable::from(structure.get(OBJECT_SCALING_ID).cloned()),
            stomach_bone_group: Scalable::from(structure.get(STOMACH_BONE_GROUP_ID).cloned()),
            torso_bone_group: Scalable::from(structure.get(TORSO_BONE_GROUP_ID).cloned()),
            upper_arm_bone_group: Scalable::from(structure.get(UPPER_ARM_BONE_GROUP_ID).cloned()),
            upper_leg_bone_group: Scalable::from(structure.get(UPPER_LEG_BONE_GROUP_ID).cloned()),
        })
    }
}
