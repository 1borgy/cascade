...
impl TryFrom<save::Ska> for Cas {
    type Error = Error;

    fn try_from(ska: save::Ska) -> Result<Self> {
        Ok(Self {
            summary: Summary::try_from(ska.summary)?,
            data: Data::try_from(ska.data)?,
        })
    }
}

impl Cas {
    pub fn modify(&self, ska: &mut save::Ska) -> Result<()> {
        self.summary.modify(&mut ska.summary);
        self.data.modify(&mut ska.data)?;
        Ok(())
    }
}
