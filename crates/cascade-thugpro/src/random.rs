...
pub fn randomize(
    ...
    let mut base_ska = save::Ska::read_from(&base_entry)?;
    let base_cas = Cas::try_from(base_ska.clone())?;

    let cases = entries
        .into_iter()
        .filter_map(|entry| save::Ska::read_from(&entry).ok())
        .filter_map(|ska| Cas::try_from(ska).ok())
