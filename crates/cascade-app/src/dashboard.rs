...
async fn load_source(entry: save::Entry) -> Result<thugpro::Cas> {
    let content = tokio::spawn(async move { thugpro::Ska::read_from(&entry) })
        .await
...
fn make_transform(source: &thugpro::cas::Cas, components: Components) -> thugpro::cas::Cas {
    thugpro::cas::Cas {
        ...
        data: thugpro::cas::Data {
            // CustomSkater
            custom_skater: source.data.custom_skater.clone().map(move |custom_skater| {
                thugpro::cas::CustomSkater {
                    // CustomSkater.custom
                    custom: custom_skater.custom.map(move |custom| {
                        thugpro::cas::Custom {
                            ...
                            scales: components.scales.then_some(custom.scales).flatten(),
                        }
                    }),
                }
            }),
            story_skater: source.data.story_skater.clone().map(move |story_skater| {
                thugpro::cas::StorySkater {
                    tricks: components.trickset.then_some(story_skater.tricks).flatten(),
                }
            }),
        },
    }
...
async fn process_entry<P: AsRef<Path>>(
    ...
    let mut ska = thugpro::Ska::read_from(&entry)?;
